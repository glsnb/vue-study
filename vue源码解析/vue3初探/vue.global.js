var Vue = (function (exports) {
  ...
  function createTransformContext(root, { prefixIdentifiers = false, hoistStatic = false, cacheHandlers = false, nodeTransforms = [], directiveTransforms = {}, isBuiltInComponent = NOOP, scopeId = null, ssr = false, onError = defaultOnError }) {
      const context = {
          // options
          prefixIdentifiers,
          hoistStatic,
  ...
  function createCodegenContext(ast, { mode = 'function', prefixIdentifiers = mode === 'module', sourceMap = false, filename = `template.vue.html`, scopeId = null, runtimeGlobalName = `Vue`, runtimeModuleName = `vue`, ssr = false }) {
      const context = {
          mode,
          prefixIdentifiers,
          sourceMap,
  ...
  function genFunctionPreamble(ast, context) {
      const { ssr, helper, prefixIdentifiers, push, newline, runtimeModuleName, runtimeGlobalName } = context;
  ...
  function createChildrenCodegenNode(branch, index, context) {
      ...
      if (needFragmentWrapper) {
          ...
          const childCodegen = firstChild.codegenNode;
  ...
  const transformElement = (node, context) => {
      ...
      return function postTransformElement() {
          ...
          const shouldUseBlock = !isComponent && (tag === 'svg' || tag === 'foreignObject');
          ...
          if (hasChildren) {
              ...
              if ( nodeType === KEEP_ALIVE && node.children.length > 1) {
...
`stop,prevent,self,`   +
  ...
  const parserOptions =  parserOptionsMinimal
      ;
  const isBuiltInDOMComponent = (tag) => {
